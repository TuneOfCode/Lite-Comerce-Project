...
namespace _20T1080009.BusinessLayers {
    ...
    public static class OrderService {
        ...
        public static bool CancelOrder(int orderID) {
            ...
            //DONE: Kiểm tra xem việc hủy bỏ đơn hàng có hợp lý đối với trạng thái hiện tại của đơn hàng hay không?
            // Đơn hàng không được huỷ (bởi khách hàng) khi trạng thái hiện tại của đơn hàng thuộc một trong các trạng thái:
        ...
        public static bool RejectOrder(int orderID) {
            ...
            //DONE: Kiểm tra xem việc từ chối đơn hàng có hợp lý đối với trạng thái hiện tại của đơn hàng hay không?
            // Đơn hàng không được từ chối (bởi nhà cung cấp) khi trạng thái hiện tại của đơn hàng thuộc một trong các trạng thái:
            // - Đơn hàng đã được duyệt trước đó: 2
            // - Đơn hàng đang được giao: 3
            // - Đơn hàng đã hoàn tất: 4
            // - Đơn hàng đã bị huỷ trước đó: -1
        ...
        public static bool AcceptOrder(int orderID) {
            ...
            //DONE: Kiểm tra xem việc chấp nhận đơn hàng có hợp lý đối với trạng thái hiện tại của đơn hàng hay không?
            //Đơn hàng được chấp nhận khi trạng thái hiện tại của đơn hàng thuộc một trong các trạng thái:
            // - Đơn hàng vừa được khởi tạo (đang chờ duyệt): 1
        ...
        public static bool ShipOrder(int orderID, int shipperID) {
            ...
            //DONE: Kiểm tra xem việc xác nhận đã chuyển hàng có hợp lý đối với trạng thái hiện tại của đơn hàng hay không?
            //Đơn hàng được xác nhận đã chuyển hàng khi trạng thái hiện tại của đơn hàng thuộc một trong các trạng thái:
            // - Đơn hàng đã được duyệt (đang chờ chuyển hàng): 2
            if (data.Status == OrderStatus.ACCEPTED) {
                data.Status = OrderStatus.SHIPPING;
